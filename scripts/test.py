...
from std_msgs.msg import Int16 as inwheel
from std_msgs.msg import Float32 as swerve
...
def pubSpeed(x, cm, stop):
    pub_speed = rospy.Publisher('inwheel_control', inwheel, queue_size=1)
    pub_angle = rospy.Publisher('swerve_control', swerve, queue_size=1)
    send_speed = inwheel()
    send_angle = swerve()
    rate = rospy.Rate(100)
    ...
    if not rospy.is_shutdown():
        print("angel : ", angle, "  speed : ", speed, " modify angle : ", -angle - 27.)
        pub_speed.publish(send_speed)
        pub_angle.publish(send_angle)
        rate.sleep()                        
